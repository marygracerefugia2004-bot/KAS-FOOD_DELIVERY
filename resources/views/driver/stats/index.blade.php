...
@section('title', 'Statistics & Insights')
@section('page-title', 'Statistics')
@section('sl-stats', 'active')
    ...
    <div class="card" style="margin-bottom: 2rem">
        ...
        <div class="card-body" style="overflow-x: auto">
            <table class="table">
                <thead>
                    <tr>
                        @foreach($weeklyEarnings as $week)
                        <th>{{ $week['week'] }}</th>
                        @endforeach
                    </tr>
                </thead>
    ...
    <div class="card">
        ...
        <div class="card-body" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1.5rem">
            <div>
                ...
                <div style="font-size: 1.8rem; font-weight: 800; color: var(--text)">{{ $stats['totalDeliveries'] }}</div>
            </div>
            <div>
                <div style="font-size: 0.85rem; color: var(--muted)">Total Earnings</div>
                <div style="font-size: 1.8rem; font-weight: 800; color: var(--orange)">₱{{ number_format($stats['totalEarnings'], 2) }}</div>
            </div>
            <div>
                <div style="font-size: 0.85rem; color: var(--muted)">Ratings</div>
                <div style="font-size: 1.8rem; font-weight: 800; color: #f5b042">{{ number_format($stats['avgRating'], 1) }} ★</div>
            </div>
            <div>
                <div style="font-size: 0.85rem; color: var(--muted)">On-Time Rate</div>
                <div style="font-size: 1.8rem; font-weight: 800; color: var(--success)">{{ $stats['onTimeRate'] }}%</div>
            </div>
        </div>
    </div>
</div>
