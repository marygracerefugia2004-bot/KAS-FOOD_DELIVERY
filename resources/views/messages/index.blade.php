...
<div class="messages-container">
    ...
    <div class="contacts-list" id="contactsList">
        @forelse($contacts as $contact)
            <a href="{{ route('messages.conversation', $contact->id) }}" class="contact-card" data-contact-name="{{ strtolower($contact->name) }}">
                <div class="contact-avatar">
                    {{ strtoupper(substr($contact->name, 0, 2)) }}
                    <!-- Example online indicator – you'd need real presence logic -->
                    <span class="online-indicator" style="background-color: {{ $contact->is_online ?? false ? '#10b981' : '#64748b' }}"></span>
                </div>
                <div class="contact-info">
                    <div class="contact-name">
                        {{ $contact->name }}
                        <span class="contact-role">{{ ucfirst($contact->role) }}</span>
                    </div>
                    <div class="last-message">
                        {{-- You need to pass last_message and last_message_time from controller --}}
                        {{ $contact->last_message ?? 'No messages yet' }}
                    </div>
                </div>
                <div class="message-meta">
                    <div class="timestamp">
                        {{ $contact->last_message_time ? \Carbon\Carbon::parse($contact->last_message_time)->diffForHumans() : '' }}
                    </div>
    ...
    <div class="modal-content">
        ...
        <div class="form-group">
            ...
            <select id="recipient_id" class="form-select">
                ...
                @foreach($allowedRecipients as $user)
