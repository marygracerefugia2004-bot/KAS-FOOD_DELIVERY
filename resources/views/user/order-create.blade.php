    ...
    <div class="card" style="margin-bottom:1.5rem;display:flex;gap:1rem;align-items:center">
        @if($food->image_path)
            @if(str_starts_with($food->image_path, 'http'))
                <img src="{{ $food->image_path }}" style="width:80px;height:80px;border-radius:10px;object-fit:cover">
            @else
                <img src="{{ asset('storage/'.$food->image_path) }}" style="width:80px;height:80px;border-radius:10px;object-fit:cover">
            @endif
        @endif
        <div>
            <div style="font-family:'Poppins',sans-serif;font-weight:700;font-size:1.1rem">{{ $food->name }}</div>
            <div style="color:var(--orange);font-size:1.2rem;font-weight:800">₱{{ number_format($food->price,2) }}</div>
            <div style="font-size:.8rem;color:var(--text-muted)"><i class="fas fa-clock"></i> ~{{ $food->prep_time + 20 }} min estimated delivery</div>
        </div>
    </div>
        ...
        <form method="POST" action="{{ route('user.order.store') }}" id="order-form">
            ...
            <div class="form-group">
                ...
                <div style="display: flex; gap: 0.5rem;">
                    <input type="text" name="delivery_address" id="address-input" class="form-control" 
                        placeholder="Click map or type address..." value="{{ old('delivery_address') }}" required style="flex: 1;">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem">
                <div class="form-group">
                    ...
                    <input type="number" name="quantity" class="form-control" min="1" max="20" value="{{ old('quantity',1) }}" required id="qty">
                </div>
                <div class="form-group">
                    <label class="form-label">Promo Code (optional)</label>
                    <input type="text" name="promo_code" class="form-control" placeholder="e.g. SAVE20" value="{{ old('promo_code') }}" style="text-transform: uppercase">
                </div>
            </div>
            
            <div class="form-group">
                <label class="form-label">Special Instructions</label>
                <textarea name="special_instructions" class="form-control" rows="2" placeholder="Allergies, special requests...">{{ old('special_instructions') }}</textarea>
            </div>
            ...
            <div style="background:var(--navy);border-radius:10px;padding:1rem;margin-bottom:1.25rem;display:flex;justify-content:space-between;align-items:center">
                ...
                <span style="color:var(--orange);font-size:1.3rem;font-weight:800" id="total-display">₱{{ number_format($food->price,2) }}</span>
            </div>
