...
<?php $__env->startSection('sl-vehicle', 'active'); ?>
...
<div style="max-width: 1100px; margin: 0 auto; padding: 2rem">
    ...
    <div class="stats-grid" style="grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); margin-bottom: 2rem">
        <div class="stat-card">
            ...
            <div class="stat-val"><?php echo e($stats['last_service'] ? \Carbon\Carbon::parse($stats['last_service'])->format('M d, Y') : 'N/A'); ?></div>
            <div class="stat-lbl">Last Service</div>
        </div>
        <div class="stat-card">
            <div class="stat-ico ico-red"><i class="fas fa-calendar-exclamation"></i></div>
            <div class="stat-val"><?php echo e($stats['next_service_due'] ? \Carbon\Carbon::parse($stats['next_service_due'])->format('M d, Y') : 'N/A'); ?></div>
            <div class="stat-lbl">Next Service Due</div>
        </div>
        <div class="stat-card">
            <div class="stat-ico ico-navy"><i class="fas fa-tachometer-alt"></i></div>
            <div class="stat-val"><?php echo e($stats['last_service_km'] ?? 'N/A'); ?> km</div>
            <div class="stat-lbl">Last Odometer</div>
        </div>
        <div class="stat-card">
            <div class="stat-ico" style="background: #D1FAE5; color: #059669"><i class="fas fa-coins"></i></div>
            <div class="stat-val">₱<?php echo e(number_format($stats['total_maintenance_cost'] ?? 0, 0)); ?></div>
    ...
    <div id="add-form" class="card" style="margin-bottom: 2rem; display: <?php echo e($shouldShowAddForm ? 'block' : 'none'); ?>">
        <div class="card-header">
            <span class="card-title"><i class="fas fa-plus-circle"></i> Add Maintenance Record</span>
        </div>
        <div class="card-body">
            <form method="POST" action="<?php echo e(route('driver.vehicle.store')); ?>" enctype="multipart/form-data">
                <?php echo csrf_field(); ?>
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem">
                    <div class="form-group">
                        <label class="form-label">Service Type *</label>
                        <input type="text" name="service_type" class="form-control" required placeholder="e.g., Oil Change" value="<?php echo e(old('service_type')); ?>">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Service Date *</label>
                        <input type="date" name="service_date" class="form-control" required value="<?php echo e(old('service_date', date('Y-m-d'))); ?>">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Odometer (km)</label>
                        <input type="number" name="odometer_km" class="form-control" min="0" value="<?php echo e(old('odometer_km')); ?>">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Next Service Due</label>
                        <input type="date" name="next_service_due" class="form-control" value="<?php echo e(old('next_service_due')); ?>">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Cost (₱)</label>
                        <input type="number" name="cost" class="form-control" step="0.01" min="0" value="<?php echo e(old('cost')); ?>">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Service Center</label>
                        <input type="text" name="service_center" class="form-control" placeholder="Shop name" value="<?php echo e(old('service_center')); ?>">
                    </div>
                </div>
                <div class="form-group" style="margin-top: 0.5rem">
                    <label class="form-label">Description / Notes</label>
                    <textarea name="description" class="form-control" rows="2" placeholder="Additional details..."><?php echo e(old('description')); ?></textarea>
                </div>
                <div class="form-group">
        ...
        <div class="table-wrap">
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td><?php echo e(\Carbon\Carbon::parse($m->service_date)->format('M d, Y')); ?></td>
                        <td><strong><?php echo e($m->service_type); ?></strong></td>
                        <td style="font-size: 0.85rem; color: var(--muted)"><?php echo e(Str::limit($m->description, 40)); ?></td>
                        <td><?php echo e($m->odometer_km ? $m->odometer_km . ' km' : '-'); ?></td>
                        <td style="color: var(--orange); font-weight: 700">₱<?php echo e(number_format($m->cost, 2)); ?></td>
                        <td><?php echo e($m->service_center ?? '-'); ?></td>
                        <td>
                            <a href="<?php echo e(route('driver.vehicle.edit', $m)); ?>" class="btn btn-sm btn-outline">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\KAS-food-deliveryY\KAS-food-delivery\resources\views\driver\vehicle\index.blade.php ENDPATH**/ ?>